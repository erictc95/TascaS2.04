...
db.restaurants.find({ "cuisine": { $ne: "American" }, "grades.grade": "A", "borough": { $ne: "Brooklyn" }}, {"_id": 0}).sort({ "cuisine": -1, "name": 1})
...
db.restaurants.find({ "grades": { $elemMatch: { "grade": "A", "score": 11, "date": ISODate("2014-08-11T00:00:00Z") } } }, {"restaurant_id": 1, "name": 1, "grades.grade": 1, "grades.score": 1, "grades.date": 1, "_id": 0})

// 23. Trobar restaurant_id, name i grades on el 2n element té grau "A", score 9 i data "2014-08-11T00:00:00Z".
db.restaurants.find({ "grades.1.grade": "A", "grades.1.score": 9, "grades.1.date": ISODate("2014-08-11T00:00:00Z") }, {"restaurant_id": 1, "name": 1, "grades.grade": 1, "grades.score": 1, "grades.date": 1, "_id": 0})
...
db.restaurants.find(
  { $or: [ { "address.street": "" }, { "address.street": null } ] },{ "address": 1, "_id": 0 })

// 29. Seleccionar documents on el valor de `coordinate` és de tipus Double. Mostrar el name, restaurant_id i coordinades.
db.restaurants.find({ "location.coordinates.0": { $type: "double" }, "location.coordinates.1": { $type: "double" } }, { "name": 1, "restaurant_id": 1, "location.coordinates": 1 , "_id": 0 })
...
db.restaurants.find({ "name": { $regex: "mon", $options: "i" } }, { "name": 1, "borough": 1, "location.coordinates": 1, "cuisine": 1, "_id": 0 })

// 32. Mostrar restaurant_id, name i grade i score de més de 80 però menys que 100.
db.restaurants.find({ grades: { $elemMatch: { score: { $gt: 80, $lt: 100 } } } }, { restaurant_id: 1, name: 1, "grades.$": 1, _id: 0 })
